...
def gen_ml_dsa_44(keyfile, passwd):
    keys.MLDSA44.generate().export_private(path=keyfile, passwd=passwd)


def gen_sphincs_plus(keyfile, passwd):
    keys.SPHINCSPlus.generate().export_private(path=keyfile, passwd=passwd)


def gen_lms(keyfile, passwd):
    keys.LMS.generate().export_private(path=keyfile, passwd=passwd)
...
keygens = {
    ...
    'ed25519':    gen_ed25519,
    'x25519':     gen_x25519,
    'ml-dsa-44':  gen_ml_dsa_44,
    'sphincs+':   gen_sphincs_plus,
    'lms':        gen_lms,
}
