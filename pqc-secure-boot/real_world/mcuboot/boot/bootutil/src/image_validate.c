...
#endif
     IMAGE_TLV_ML_DSA_44,
     IMAGE_TLV_SPHINCS_PLUS,
     IMAGE_TLV_LMS,
     IMAGE_TLV_PQC_PUBKEY,
                     ...
                     )
{
    ...
    static uint8_t pqc_pubkey_buf[PQC_MAX_PUBKEY_LEN];
    uint16_t pqc_pubkey_len = 0;
#ifdef EXPECTED_SIG_TLV
    ...
    while (true) {
...
#endif /* EXPECTED_SIG_TLV */
        case IMAGE_TLV_PQC_PUBKEY:
        {
            BOOT_LOG_DBG("bootutil_img_validate: PQC PUBKEY TLV");
            if (len > sizeof(pqc_pubkey_buf)) {
                rc = -1;
                goto out;
            }
            rc = LOAD_IMAGE_DATA(hdr, fap, off, pqc_pubkey_buf, len);
            if (rc) {
                goto out;
            }
            pqc_pubkey_len = len;
        ...
        case IMAGE_TLV_LMS:
        {
            BOOT_LOG_DBG("bootutil_img_validate: PQC SIG TLV type 0x%04x", type);
            static uint8_t pqc_sig_buf[PQC_MAX_SIG_LEN];
            if (len > sizeof(pqc_sig_buf)) {
                rc = -1;
                goto out;
            }
            rc = LOAD_IMAGE_DATA(hdr, fap, off, pqc_sig_buf, len);
            if (rc) {
                goto out;
            }
            FIH_CALL(bootutil_verify_pqc, valid_signature, type, hash, sizeof(hash),
                     pqc_sig_buf, len, pqc_pubkey_buf, pqc_pubkey_len);
